...
isScaleInExpressedTypeRange(function_ref<InFlightDiagnostic()> emitError,
  ...
  double minScale = -maxScale; // We allow negative scales
...
/// Parses a bracketed list of quantization parameters, returning the
/// dimensions of the parsed sub-tensors in `dims`. The dimension of the
/// list is prepended to the dimensions of the sub-tensors. This function
/// assumes that the initial left brace has already been parsed. For
/// example:
///
///   parseQuantParamListUntilRBrace(1.0:1, 2.0:4, 3.0:4}) -> Success,
///       dims = [3], scales = [1.0, 2.0, 3.0], zeroPoints = [1, 4, 4]
///
///   parseQuantParamListUntilRBrace({1.0, 2.0}, {3.0:1, 4.0:9}}) ->
///   Success,
///       dims = [2, 2], scales = [1.0, 2.0, 3.0, 4.0], zeroPoints = [0, 0,
///       1, 9]
///
