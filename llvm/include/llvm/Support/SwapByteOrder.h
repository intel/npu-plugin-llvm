...
inline unsigned long getSwappedBytes(unsigned long C) {
  ...
  return static_cast<unsigned long>(sizeof(long) == sizeof(int) ? ByteSwap_32((uint32_t)C)
                                                                : ByteSwap_64((uint64_t)C));
}
inline signed long getSwappedBytes(signed long C) {
  // Handle LLP64 and LP64 platforms.
  return static_cast<signed long>(sizeof(long) == sizeof(int) ? ByteSwap_32((uint32_t)C)
                                                              : ByteSwap_64((uint64_t)C));
}
